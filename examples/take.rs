...
async fn main(_spawner: async_main::LocalSpawner) {
    let mut ei = event_iterator::from_iter_ref([1, 2, 3]).take(2);
    ...
    let mut ei = event_iterator::from_iter_ref(0..).take(3);
    ...
    let mut ei = event_iterator::from_iter_ref([1, 2]).take(5);
